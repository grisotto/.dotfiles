...
-- linha do painel: anotar a linha que está sendo lida. Elas são globais, e no
-- grupo `<Leader>g` há letras que *parecem* livres e não estão: o gitsigns
-- ocupa as óbvias com mapeamentos locais ao buffer, que ganham do global em
...
-- anotando.
--
-- `<Leader>ga` e `<Leader>gA` foram conferidos com `nvim_buf_get_keymap` num
-- arquivo rastreado desta configuração (o `nvim_get_keymap` só enxerga os
-- globais): livres nos dois lados. Ocupados hoje estão `gg`, `gb`, `gc`, `gC`,
-- `gt`, `gT`, `go` e o grupo `gn` do neogit, mais os locais do gitsigns
-- (`gl`, `gL`, `gp`, `gr`, `gR`, `gs`, `gS`, `gd`).
--
